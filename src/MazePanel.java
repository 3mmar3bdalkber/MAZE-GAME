...
public class MazePanel extends JPanel implements KeyListener {
    ...
    public MazePanel(RightSidePanel rightSidePanel) {
       this.rightSidePanel=rightSidePanel;
        level=new Level(1,"Easy",new Color(0, 180, 100));
        rows=level.getRows();
        cols=level.getCols();
        goalCol=cols-1;
        goalRow=rows-1;
        ...
        rightSidePanel.setGameOverListener(this::onLoss);
    ...
    private void drawMaze(Graphics2D g2) {
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                ...
                if (generator.wallsTop[r][c]) g2.drawLine(x, y, x + CELL_SIZE, y);
                if (generator.wallsRight[r][c]) g2.drawLine(x + CELL_SIZE, y, x + CELL_SIZE, y + CELL_SIZE);
                if (generator.wallsBottom[r][c]) g2.drawLine(x, y + CELL_SIZE, x + CELL_SIZE, y + CELL_SIZE);
                if (generator.wallsLeft[r][c]) g2.drawLine(x, y, x, y + CELL_SIZE);
            }
        }
    }
